		...
		try {
			while (!quitter) {
				...
				while (!(data==10)) {
					...
					donnee = network.read(); // Lecture de la donnée à partir du réseau
					data = (int) donnee; // Conversion de la donnée en entier

					switch(data) {
					    case 1: // Scan des briques
					        LCD.clear();
					        LCD.drawString("Scan", 0, 4);
					        ev3Controller.startScanningProcess(); // Démarre le processus de scan des briques
					        LCD.drawString("Fin scan", 0, 5);
					        Delay.msDelay(750);
					        break;
					        
					    case 2: // Affiche le contenu des bacs
					        LCD.clear();
					    ...
					    case 3: // Trier tout
					        ...
					        ev3Controller.sortAllBricksOnSlide(); // Trie toutes les briques sur la rampe
					        LCD.drawString("Fin tri", 0, 5);
					        Delay.msDelay(750);
					        break;
					        
					    case 5: // Reset de la rampe
					        LCD.clear();
					        LCD.drawString("MAJ Rampe", 0, 4);
					        ev3Controller.resetRampe(); // Réinitialise la position de la rampe
					        LCD.drawString("Fin MAJ", 0, 5);
					        Delay.msDelay(750);
					        break;
					        
					    case 4: // Jeter tout à la poubelle
					        LCD.clear();
					        LCD.drawString("Poubelle", 0, 4);
					        ev3Controller.ejectToTrash(); // Éjecte toutes les briques dans la poubelle
					...
					Delay.msDelay(250); // Attente de 250 millisecondes avant de continuer l'exécution
